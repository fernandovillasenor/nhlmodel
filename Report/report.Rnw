...
The point system of the NHL currently awards two points to the winner, one point to a team that loses either in overtime or in a shoot-out, and zero points to a team that loses in regulation time. From a broad range of variables including (but not limited to): Goals Scored, Goals Allowed, Shots Taken, Shots Allowed, Age of Players, we an attempt to explain end of season point totals for individual teams by way of linear regression. For our analysis, we do not make use of particular data from any individual game, as we are using aggregate quanitities to determine a teams current season standing.

Our data ranges from the 2007--2008 season to the 2014--2015 season. Before the 2007--2008 season many of our advanced metrics were not available. A peculiarity of our data is that the 2012--2013 was affected by a lockout which led to only 48 out of a usual 82 games played. Despite this apparent mixing of data we still use all of this season in our analysis and deal with this problem by scaling certain covariates by the proportion of the season in which a team has played.

\section{Exploratory analysis}

Many of the variables in hockey data are redundant. Points summarizes the number of wins, losses, and overtime or shootout losses a team has, while faceoff percentage summarizes the number of faceoffs a team has won and lost. In selecting appropriate predictor data, our aim was to choose variables that conveyed different information to avoid colinearity. These variables are shown below.
...
% I added in multiplication symbols because when I was reading it on paper it was initially hard to make out what the scope of predictor formula was.
\vspace{-25mm}\noindent
...
Not all of the variables increase as the number of games played increases. In particular, AvAge, FF\%, PK\%, PP\%, SOS and SRS are relatively stable as the number of games played changes. This is evident by comparing the value of PK\% in the 48 game season to the value of PK\% in the 82 game season in Table 2. Later on in our analysis, when we predict points in the 2015--2016 season, we scale only the coefficients of the variables for which we expect to see increase by the proportion of the season a team has played. We then obtain the out of sample Mean Squared Error of these predictions.

\section{Methods}

To explain total points accumulated for a specific team we used multivariate regression. We used the R function from the \texttt{leaps} package to determine the best model given a certain number of covariates. This function tried all possible combinations of the variables shown in Table 1 given a fixed model size and returned the variables which were most significant (had the smallest $p$-values). We then compared models of different sizes using $R^{2}_{Adj}$ and out of sample MSE (MSPE). Wep based our current season points predictions as at November 21, 2015.
...
\end{tabular} & \Sexpr{round(sm2.ars,4)} & \Sexpr{round(mse2, 4)} \\ \hline
...
\end{tabular} & \Sexpr{round(sm4.ars,4)} & \Sexpr{round(mse4, 4)} \\ \hline
...
\end{tabular} & \Sexpr{round(sm6.ars,4)} & \Sexpr{round(mse6, 4)} \\ \hline
...
\end{tabular} & \Sexpr{round(sm8.ars,4)} & \Sexpr{round(mse8, 4)} \\ \hline
\end{tabular}
\caption{Models fit to data with 2012--2013 lockout season included}
\end{table}

\medskip\noindent When models were fit to the first training set, we can see that goals for (GF) and goals against (GA) are the most important variables followed by shots (S) and shots against (SA). Most of the coefficients have an intuitive explanation. For instance, if a team scores many goals they have a better chance of winning games and getting more points so the coefficient for goals for is positive. There are notable interesting coefficients such as the coefficient for average age, which is positive. One interpretation of the posisitive average age coefficient might imply that teams with older players tend to do better than teams with younger players since they have more experience.
...
\end{tabular} & \Sexpr{round(sm3nl.ars,4)} & \Sexpr{round(mse3nl, 4)} \\ \hline
